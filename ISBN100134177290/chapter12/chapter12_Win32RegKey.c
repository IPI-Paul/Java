/**
 	chapter12_Win32RegKey.c Listing 12.22
	Win32RegKey class Listing 12.21 <br />
 	Win32RegKeyNameEnumeration inner class implements Enumeration<String> Listing 12.21 
 	Win32RegKeyException inner class extends RuntimeException Listing 12.21 
 	chapter12_Win32RegKey.h Listing 12.21
	Win32RegKeyTest class Listing 12.23
	@version 1.00 1997-07-01
	@author Cay Horstmann
*/

#include "chapter12_Win32RegKey.h"
#include "chapter12_Win32RegKeyNameEnumeration.h"
#include <string.h>
#include <stdlib.h>
#include <windows.h>

JNIEXPORT jobject JNICALL Java_chapter12_Win32RegKey_getValue(JNIEnv* env, jobject this_obj, jobject name) {
	const char* cname;
	jstring path;
	const char* cpath;
	HKEY hkey;
	...
	if (RegOpenKeyEx(root, cpath, 0, KEY_READ, &hkey) != ERROR_SUCCESS) {
		(*env) -> ThrowNew(env, (*env) -> FindClass(env, "chapter12/Win32RegKeyException"),
			"Open key failed");
...
JNIEXPORT void JNICALL Java_chapter12_Win32RegKey_setValue(JNIEnv* env, jobject this_obj, jstring name, 
		jobject value) {
	const char* cname;
	jstring path;
	const char* cpath;
...
static int startNameEnumeration(JNIEnv* env, jobject this_obj, jclass this_class) {
	...
	if (RegQueryInfoKey(hkey, NULL, NULL, NULL, NULL, NULL, NULL, &count, &maxsize, NULL, NULL, NULL) != 
			ERROR_SUCCESS) {
		(*env) -> ThrowNew(env, (*env) -> FindClass(env, "chapter12/Win32RegKeyException"),
...
JNIEXPORT jboolean JNICALL Java_chapter12_Win32RegKeyNameEnumeration_hasMoreElements(JNIEnv* env, 
		jobject this_obj) {
	jclass this_class;
	jfieldID id_index;
	jfieldID id_count;
...
JNIEXPORT jobject JNICALL Java_chapter12_Win32RegKeyNameEnumeration_nextElement(JNIEnv* env, 
		jobject this_obj) {
	jclass this_class;
	jfieldID id_index;
	jfieldID id_hkey;
