...
import ipi.FolderPaths;

/**
 * {@code ImageListDnDFrame} class extends JFrame Listing 11.22 <br />
...
public class ImageListDnDFrame extends JFrame {
	...
	public ImageListDnDFrame() {
		...
		*/
		list1 = new ImageList(Paths.get(FolderPaths.getPath(),"sourceFiles", "images"));
		list2 = new ImageList(Paths.get(System.getProperty("user.home"),"Pictures"));
...
/**
 * This class is both a drag source and a drop target.
 */
	...
	public ImageList(Path dir) {
		...
		try (DirectoryStream<Path> entries = Files.newDirectoryStream(dir)) {
			for (Path entry : entries) {
				ImageIcon img = new ImageIcon(entry.toString());
				if (img.getIconWidth() > 100) 
					img = new ImageIcon(img.getImage().getScaledInstance((int) 100, -1, Image.SCALE_SMOOTH));
