package chapter10;  // internalFrame
...
/**
 * {@code DesktopFrame} class extends JFrame Listing 10.31 <br />
...
public class DesktopFrame extends JFrame {
	...
	public DesktopFrame() {
		...
		openItem.addActionListener(event -> {
			...
			createInternalFrame(label, planets[counter]);
			label.setIcon(new ImageIcon(new ImageIcon(ResourcePaths.getResource("img", 
					planets[counter] + ".jpg"))
					.getImage().getScaledInstance((int) (label.getWidth() / 1.5), -1, Image.SCALE_SMOOTH)));
		...
		dragOutlineItem.addActionListener(event -> desktop.setDragMode(dragOutlineItem.isSelected() ? 
	...
	public void createInternalFrame(Component c, String t) {
		...
		desktop.add(iframe);		
		iframe.setFrameIcon(new ImageIcon(ResourcePaths.getResource("img", "document.gif")));
	...
	public void cascadeWindows() {
		...
		for (JInternalFrame frame : desktop.getAllFrames()) {
			if (!frame.isIcon()) {
				try {
					//try to make maximized frames resizeable; this might be vetoed
